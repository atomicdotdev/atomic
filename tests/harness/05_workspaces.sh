#!/usr/bin/env bash
# 05_workspaces.sh — Per-view workspace isolation tests.
#
# Workspaces are the mechanism by which views achieve FULL working copy
...
# view switch  → shelves current working state, unshelves target
# record/add/status → work exactly as before, always from project root
#
# Invariants tested:
#
#   1. view create creates .atomic/workspaces/<name>/
#   2. switch_view shelves ignored/artifact files into the old workspace
#   3. switch_view restores ignored/artifact files from the new workspace
#   4. Ignored files on view A do NOT appear after switching to view B
#   5. Switching back to view A restores its ignored files exactly
...
# ═══════════════════════════════════════════════════════════════════════════
begin_section "Workspace: view create creates workspace storage"
# ═══════════════════════════════════════════════════════════════════════════

make_temp_repo "ws-create"
init_repo

# dev is the initial view — it should have workspace storage
assert_dir_exists \
...
assert_dir_exists \
    ".atomic/workspaces/feature created with view" \
...
# Record a tracked file on dev so the view has content
...
# Tracked file should exist (inherited via insert)
...
# ═══════════════════════════════════════════════════════════════════════════
begin_section "Workspace: Each view has its own artifacts"
# ═══════════════════════════════════════════════════════════════════════════
#
# Workflow:
...
# by the graph — record, switch, insert all work as before.
...
# workspace combine to give the complete working copy for a view.
...
# ═══════════════════════════════════════════════════════════════════════════
begin_section "Workspace: Three views with divergent artifacts"
# ═══════════════════════════════════════════════════════════════════════════
#
...
new_view "agent-a" >/dev/null 2>&1 || true
insert_from_view "dev" "agent-a" >/dev/null 2>&1 || true
switch_view "agent-a" >/dev/null 2>&1 || true
...
switch_view "dev" >/dev/null 2>&1 || true
new_view "agent-b" >/dev/null 2>&1 || true
insert_from_view "dev" "agent-b" >/dev/null 2>&1 || true
switch_view "agent-b" >/dev/null 2>&1 || true
...
# ── Verify each view sees its own deps ──
...
# ═══════════════════════════════════════════════════════════════════════════
#
# Switch between views 5 times to verify no state corruption or leakage.
...
_pass "rapid switching: 5 round-trips across 3 views completed"

# ═══════════════════════════════════════════════════════════════════════════
begin_section "Workspace: Empty view has no artifacts"
# ═══════════════════════════════════════════════════════════════════════════
#
# A freshly created view with no history should have an empty working
# copy — no tracked files AND no artifacts.

make_temp_repo "ws-empty-view"
init_repo
...
# Create empty view and switch
new_view "empty" >/dev/null 2>&1 || true
switch_view "empty" >/dev/null 2>&1 || true

# In the ambient graph model, child views see parent's files through
# the filter chain — the view is parented on dev so main.ts is visible.
assert_file_exists "main.ts visible on child view (inherited from dev)" "main.ts"

# No artifacts (workspace is fresh)
assert_dir_not_exists "no node_modules on empty view" "node_modules"
...
new_view "agent-review-test" >/dev/null 2>&1 || true
insert_from_view "dev" "agent-review-test" >/dev/null 2>&1 || true
switch_view "agent-review-test" >/dev/null 2>&1 || true
...
# Human switches to agent's view to review
switch_view "agent-review-test" >/dev/null 2>&1 || true
...
assert_file_content \
    ...
    "dist/app.js" \
    "compiled_agent"
assert_file_not_exists "no human deps on agent view" "node_modules/humanlib/index.js"
...
# ═══════════════════════════════════════════════════════════════════════════
begin_section "Workspace: .atomicignore changes between views"
# ═══════════════════════════════════════════════════════════════════════════
#
# Different views might have different .atomicignore rules (e.g. agent
...
# ═══════════════════════════════════════════════════════════════════════════
begin_section "Workspace: View deletion cleans workspace"
# ═══════════════════════════════════════════════════════════════════════════
#
# When a view is deleted, its workspace storage should be cleaned up too.
...
new_view "ephemeral" >/dev/null 2>&1 || true
insert_from_view "dev" "ephemeral" >/dev/null 2>&1 || true
switch_view "ephemeral" >/dev/null 2>&1 || true

# Create artifacts on ephemeral view
mkdir -p node_modules
...
# Delete the ephemeral view
del_out="$(atomic view delete ephemeral 2>&1)" || true
if echo "$del_out" | grep -qiE "deleted|removed|success"; then
    _pass "delete ephemeral view"
else
    _pass "delete ephemeral view completes"
fi

# Workspace storage should be cleaned up
assert_dir_not_exists \
    "ephemeral workspace removed after view delete" \
    ".atomic/workspaces/ephemeral"

# ═══════════════════════════════════════════════════════════════════════════
begin_section "Workspace: Many views stress test"
# ═══════════════════════════════════════════════════════════════════════════
#
# Create 5 views, each with unique artifacts.  Switch between them
...
# Create 5 views with unique node_modules
for i in $(seq 1 5); do
    new_view "agent-${i}" >/dev/null 2>&1 || true
    insert_from_view "dev" "agent-${i}" >/dev/null 2>&1 || true
    switch_view "agent-${i}" >/dev/null 2>&1 || true

    mkdir -p node_modules
    create_file "node_modules/marker.txt" "agent-${i}-deps"
done

# Now cycle through all views and verify each has its own marker
for i in $(seq 1 5); do
    switch_view "agent-${i}" >/dev/null 2>&1 || true
    assert_file_content \
...
# One more round trip
switch_view "agent-3" >/dev/null 2>&1 || true
assert_file_content \
    "agent-3 still has its deps after full cycle" \
    "node_modules/marker.txt" \
    "agent-3-deps"

_pass "5-view stress test completed"
