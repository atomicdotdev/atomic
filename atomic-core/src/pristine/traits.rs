...
use crate::types::{
    ChangePosition, EdgeFlags, ForwardEdge, GraphNode, Hash, Inode, Merkle, NodeId, ParentEdge,
    Position, SerializedGraphEdge,
};
...
pub trait GraphTxnT {
    ...
    ///
    /// ```ignore
    /// use atomic_core::types::EdgeKind;
    ///
    /// for edge in txn.iter_forward(node, false)? {
    ///     match edge.kind {
    ///         EdgeKind::Block | EdgeKind::PseudoBlock => { /* content */ }
    ///         EdgeKind::Folder | EdgeKind::PseudoFolder => { /* directory */ }
    ///         EdgeKind::BlockDeleted | EdgeKind::FolderDeleted => unreachable!(),
    ///     }
    /// }
    /// ```
    fn iter_forward(
    ...
    ///
    /// ```ignore
    /// use atomic_core::types::ParentEdgeKind;
    ///
    /// for edge in txn.iter_parents(node, true)? {
    ///     match edge.kind {
...
mod tests {
    use super::*;
    use crate::types::{Edge, EdgeKind, ForwardEdge, ParentEdge, ParentEdgeKind};
