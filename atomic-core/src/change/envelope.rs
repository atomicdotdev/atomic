...
/// Errors from encoding or decoding a [`SessionEnvelope`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SessionEnvelopeError {
    /// What went wrong during encode/decode.
    pub reason: String,
}

impl fmt::Display for SessionEnvelopeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "session envelope codec error: {}", self.reason)
    }
}

impl std::error::Error for SessionEnvelopeError {}

/// Result type for session envelope codec operations.
pub type EnvelopeResult<T> = std::result::Result<T, SessionEnvelopeError>;

fn codec_error(reason: impl Into<String>) -> SessionEnvelopeError {
...
impl SessionEnvelope {
    ...
    /// # Errors
    ///
    /// Returns `SessionEnvelopeError` if postcard serialization fails.
    pub fn encode(&self) -> EnvelopeResult<Vec<u8>> {
        let payload = postcard::to_allocvec(self)
            .map_err(|e| codec_error(format!("postcard serialize failed: {}", e)))?;
    ...
    pub fn decode(data: &[u8]) -> EnvelopeResult<Self> {
        ...
        let envelope: Self = postcard::from_bytes(&data[4..])
            .map_err(|e| codec_error(format!("postcard deserialize failed: {}", e)))?;
...
mod tests {
    ...
    fn test_decode_corrupted_payload() {
        ...
        let err = SessionEnvelope::decode(&bytes).unwrap_err();
        assert!(matches!(err, SessionEnvelopeError { .. }));
    }
