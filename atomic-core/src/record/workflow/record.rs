...
use super::crdt::{
    CrdtBuildStats, CrdtChangeBuilder, ContentTokenizer,
};
...
impl RecordedFile {
    ...
    pub fn is_directory(&self) -> bool {
        matches!(self.kind, Some(DetectionKind::Added)) && self.content.is_empty() && self.hunks.is_empty()
    }

    /// Check if this recorded file represents a deleted directory.
    #[must_use]
    pub fn is_deleted_directory(&self) -> bool {
        matches!(self.kind, Some(DetectionKind::Deleted)) && self.content.is_empty() && self.hunks.is_empty()
    }
...
fn build_crdt_ops_for_added_file(
    ...
    let builder_file_op = file_ops
        .into_iter()
        .next()
        .unwrap_or_else(|| BuilderFileOps::new(TrunkId::new(placeholder_change_id, 0), path.to_string(), None));
    ...
    W: WorkingCopyRead,
{
    ...
    for mut graph_op in hunk_result.into_hunks() {
...
fn build_crdt_ops_for_modified_file(
    ...
    for op in &line_diff.diff_ops {
        match op {
            crate::diff::DiffOp::Equal { old_pos, new_pos, len } => {
            ...
            crate::diff::DiffOp::Delete { old_pos, new_pos: _, len } => {
            ...
            crate::diff::DiffOp::Insert { old_pos: _, new_pos, len } => {
                ...
                for i in 0..*len {
                    ...
                    if line_idx < new_lines.len() {
                        ...
                        let line_op = BuilderLineOps::insert(branch_id, prev_branch, leaf_ops)
                            .with_new_line_num(line_idx + 1); // 1-indexed line number
                        file_ops.add_line_op(line_op);
                        stats.lines_added += 1;
            ...
            crate::diff::DiffOp::Replace { old_pos, old_len, new_pos, new_len } => {
                // Replaced lines - delete old and insert new with token-level ops
                // First, delete old lines with their original content
                for i in 0..*old_len {
                    let line_idx = old_pos + i;
                    let branch_id = alloc_branch();
                    ...
                    let line_op = BuilderLineOps::delete(branch_id, content)
                        .with_old_line_num(line_idx + 1); // 1-indexed line number
                    file_ops.add_line_op(line_op);
                    stats.lines_deleted += 1;
                }

                // Then, insert new lines with tokens
                for i in 0..*new_len {
                    let line_idx = new_pos + i;
                    if line_idx < new_lines.len() {
                        let line = &new_lines[line_idx];
